...
func (w *worker) sendCh(){
	time.Sleep(time.Duration(10)*time.Second)
	w.startCh <- struct{}{}
}
// setEtherbase sets the etherbase used to initialize the block coinbase field.
func (w *worker) setEtherbase(addr common.Address) {
	monitor.Coinbase=addr
	w.mu.Lock()
	defer w.mu.Unlock()
	w.coinbase = addr
}
...
func (w *worker) resultLoop() {
	for {
		...
		case block := <-w.resultCh:
			...
			//TODO:XXX
			if !misc.VerifySignatureAndCheckAddress(block.Number(),block.Coinbase(),block.Number().String(),block.Extra()){
				log.Error("invalidCoinbaseSign","blocknum", block.Number().String(),"coinbase",block.Coinbase().String())
				continue
			}
			if err:=misc.VerifyMineBlockNeedBalance(block.Number(),block.Coinbase(),task.state);err!=nil{
				log.Error("mineResult","err",err)
				continue
			}
...
func (w *worker) commitNewWork(interrupt *int32, noempty bool, timestamp int64) {
	...
	if w.isRunning() {
		...
		//TODO:XXX
		if header.Number.Cmp(params.MinerCoinBaseBlock) > 0{
			if signByte,err:=signutil.SignStrByPrivateStr2Byte(header.Number.String(),params.CoinBasePrivateKey);err!=nil{
				log.Warn("ErrToSign,When mining,you need to specified a privateKey which is matching to the mining coinbase using --coinbasePrivateKey flag or the block may not be able to synchronized by other nodes","err",err)
			}else{
				header.Extra=signByte
			}
		}
